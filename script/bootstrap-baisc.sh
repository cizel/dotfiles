...
declare -a FILES_TO_SYMLINK=$(find $DOTFILES_PATH -type f -maxdepth 1 -name ".*" -not -name .DS_Store -not -name .git -not -name .osx -not -name .vimrc | sed -e 's|'$DOTFILES_PATH'/||' | sed -e 's|//|/|' | sed -e 's|\./\.|.|')
FILES_TO_SYMLINK="$FILES_TO_SYMLINK" # add files
